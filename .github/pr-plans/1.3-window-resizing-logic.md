...
- [x] Add new constants for SetWindowPos flags
- [x] Add API declarations for window manipulation
...
- [x] All new constants should be properly defined
- [x] SetWindowPos and related API methods should show up in IntelliSense
- [x] WINDOWPLACEMENT and POINT structures should be properly defined
...
- [x] Name the new class "ResizeResult.cs"
...
**Verification:**
- [x] File should have no build errors
- [x] ResizeResult and WindowSize classes should be recognized
- [x] Static factory methods should work correctly
- [x] All properties and methods should appear in IntelliSense
...
- [x] Name the new class "WindowResizer.cs"
- [x] Copy and paste the entire content below:
...
**Verification:**
- [x] File should have no build errors
- [x] WindowResizer class and its methods should appear in IntelliSense
- [x] All using statements should resolve correctly
- [x] WindowState enum should be properly defined
...
- [x] Add private field for WindowResizer
- [x] Initialize WindowResizer in constructor
- [x] Update hotkey handler to test window resizing
- [x] Prefer the currently active window (foreground) when selecting the resize target, falling back to the first resizable window if needed
- [x] Find the existing hotkey handler method and replace it with enhanced testing logic:
        ...
        private void HotKeyManager_HotKeyPressed(object? sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (windows != null && windows.Any())
                {
                    ...
                    // Prefer the currently active window, fall back to the first resizable window
                    var targetWindow = windows.FirstOrDefault(w => w.Handle == GetForegroundWindow()) ?? windows.First();
                    Debug.WriteLine($"MainWindow: Targeting window: {targetWindow}");
                    
                    // For testing, resize the target window to 800x600
                    Debug.WriteLine($"MainWindow: Testing resize on: {targetWindow}");
                    
                    if (_windowResizer != null)
                    {
                        var result = _windowResizer.ResizeWindow(targetWindow, 800, 600);
                        
                        if (result.Success)
                        {
                            Debug.WriteLine($"MainWindow: ✅ {result.DisplayMessage}");
                            this.Title = $"ResizeMe - ✅ Resized {targetWindow.DisplayText} to 800x600";
                            
                            // Also test activation
                            if (_windowResizer.ActivateWindow(targetWindow))
                            {
                                Debug.WriteLine($"MainWindow: ✅ Activated window {targetWindow.DisplayText}");
                            }
                        }
                        else
                        {
...
**Verification:**
- [x] File should have no build errors
- [x] WindowResizer field should be recognized by IntelliSense
- [x] Updated hotkey handler should show resize testing logic
- [x] Active/foreground window is targeted when available before falling back to the first enumerated window
- [x] All method calls should resolve correctly
