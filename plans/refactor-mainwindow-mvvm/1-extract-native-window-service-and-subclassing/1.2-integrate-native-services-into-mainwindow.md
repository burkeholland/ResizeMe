...
- [x] **Current Step:** Step 1.1 completed (services exist)
- [x] **Codebase State:** Clean build: `dotnet build ResizeMe.sln`
...
- [x] At top of `MainWindow.xaml.cs`, add `using ResizeMe.Native;`
- [x] Introduce private readonly fields:
...
- [x] Remove `_messageBridge`, `_subclassProc`, `_subclassId`, `_subclassAttached` fields—they are now encapsulated.

- #### Step 2: Replace handle + shell code
- [x] Replace `EnsureWindowHandle` body with service calls:
...
- [x] Update `ConfigureShell` to defer to service:
...
- [x] Replace `AttachSubclass` contents with:
...
- [x] Delete nested `WindowMessageBridge` class and replace with `WindowMessageHandler` implementing `IWindowMessageHandler` (same message logic but without subclass plumbing):
...
- [x] Replace direct calls to `WindowsApi.ShowWindow` / `SetForegroundWindow` with service equivalents:
...
- [x] Update `HideWindow` to call `_nativeWindowService.HideWindow(_windowHandle);`

#### Step 5: Dispose subclass service
- [x] In `OnClosed`, call `_subclassService.Detach();`
- [x] Implement `IDisposable` on `MainWindow` or override `OnClosed` to dispose `_subclassService` (already detaching is enough because service implements `IDisposable`).

## Verification Checklist

- [x] `MainWindow.xaml.cs` no longer contains nested `WindowMessageBridge`/`WinApiSubClass`
- [x] `dotnet build ResizeMe.sln` passes
- [x] Hotkey + tray clicks still route through handler when running locally
