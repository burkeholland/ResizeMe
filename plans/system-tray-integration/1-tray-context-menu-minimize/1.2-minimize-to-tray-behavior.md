...
- [ ] **Build OK:** `dotnet build ResizeMe.sln`
 ...
 - [x] **Prior Substep:** 1.1 completed (TrayIconManager updated)
...
- [ ] Add constants for messages & system command
 ...
 - [x] Add constants for messages & system command
...
- [ ] In `EnsureHotKeyRegistration()` after tray initialization, subscribe to events:
 - [x] In `EnsureHotKeyRegistration()` after tray initialization, subscribe to events:
...
- [ ] Modify `WndProcSubClass` to process tray callback message and system close:
 - [x] Modify `WndProcSubClass` to process tray callback message and system close:
...
- [ ] You may keep original `SettingsButton_Click`; the tray menu uses `OpenSettingsWindow()`.
 - [x] You may keep original `SettingsButton_Click`; the tray menu uses `OpenSettingsWindow()` — deduplicated by delegating `SettingsButton_Click` to `OpenSettingsWindow()`.
